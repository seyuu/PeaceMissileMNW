...
WEBHOOK_BASE_URL = os.environ.get("WEBHOOK_BASE_URL")  # sonunda slash var!
...
def update_user_score(user_id, new_score):
    try:
        logger.info(f"update_user_score çağrıldı: user_id={user_id}, new_score={new_score}")
        ...
        logger.info(f"Firestore güncellemesi: current_max={current_max}, total_with_bonus={total_with_bonus}, total_coins={total_coins}")
        ref.update({
            'score': current_max,
